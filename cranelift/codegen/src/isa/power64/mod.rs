//! power 64-bit Instruction Set Architecture.
#![allow(unused)]

use crate::dominator_tree::DominatorTree;
use crate::isa::power64::settings as power64_settings;
...
/// An power64 backend.
pub struct Power64Backend {
    triple: Triple,
    flags: shared_settings::Flags,
    isa_flags: power64_settings::Flags,
}


impl Power64Backend {
    /// Create a new power64 backend with the given (shared) flags.
    ...
    fn compile_vcode(
        ...
        let sigs = SigSet::new::<abi::Power64MachineDeps>(func, &self.flags)?;
        let abi = abi::Power64Callee::new(func, self, &self.isa_flags, &sigs)?;
        compile::compile::<Power64Backend>(func, domtree, self, abi, emit_info, sigs, ctrl_plane)
    }
}

impl TargetIsa for Power64Backend {
    fn compile_function(
        &self,
        func: &Function,
        domtree: &DominatorTree,
...
impl fmt::Display for Power64Backend {
...
pub fn isa_builder(triple: Triple) -> IsaBuilder {
    ...
    IsaBuilder {
        triple,
        setup: power64_settings::builder(),
...
fn isa_constructor(
    ...
    let isa_flags = power64_settings::Flags::new(&shared_flags, builder);

    let backend = Power64Backend::new_with_flags(triple, shared_flags, isa_flags);
    Ok(backend.wrapped())
}
